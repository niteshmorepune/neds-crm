...
it('creates a weekly_monday task on a Monday', function () {
    ...
    $service = Service::factory()->create(['name' => 'Google Ads']);
    ...
    expect(Task::where('project_id', $project->id)->where('title', 'Google Ads performance review')->exists())->toBeTrue();
});
...
it('does not create a weekly_monday task on a non-Monday', function () {
    ...
    expect(Task::where('project_id', $project->id)->where('title', 'Google Ads performance review')->exists())->toBeFalse();
});
...
it('assigns the task to the project lead (pivot role=lead) not the owner', function () {
    ...
    $task = Task::where('project_id', $project->id)->where('title', 'Google Search Console review')->first();
...
it('skips projects with no matching service template', function () {
    ...
    // AI Automation only has AMC + contacts audit templates — no weekly tasks
    $this->artisan('app:dispatch-scheduled-tasks', ['--date' => '2026-07-06'])->assertSuccessful(); // Monday
...
it('creates tasks for multiple projects independently', function () {
    ...
    expect(Task::where('title', 'GMB profile health check')->count())->toBe(2);
});
