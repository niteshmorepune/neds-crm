<x-app-layout>
    <x-slot name="header">{{ $offer->shortLabel() }} — {{ $stageLabel }}</x-slot>
    ...
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        <div>
            <h3 class="text-sm font-semibold uppercase tracking-wide text-gray-500 mb-2">
                {{ $offer->shortLabel() }} — {{ $stageLabel }} ({{ $leads->count() }})
            </h3>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($leads as $lead)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-gray-600">{{ $lead->goal?->label() ?? '—' }}</td>
                                <td class="px-4 py-3 text-gray-600">{{ $lead->budget_range?->label() ?? '—' }}</td>
                                <td class="px-4 py-3 text-gray-600">{{ $lead->owner?->name ?? '—' }}</td>
                                <td class="px-4 py-3 text-gray-600">{{ $lead->recommendation_generated_at?->diffForHumans() }}</td>
                        ...
                        @empty
                            <tr><td colspan="7" class="px-4 py-6 text-center text-gray-400">No leads at this stage in this window.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
