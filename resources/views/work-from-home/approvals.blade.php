<x-app-layout>
    <x-slot name="header">WFH Approvals</x-slot>
    ...
    <div class="max-w-4xl mx-auto space-y-4" x-data="{ rejecting: null }">
        ...
        <div class="flex flex-wrap items-center justify-between gap-2">
            <a href="{{ route('work-from-home.index') }}" class="text-sm text-indigo-600 hover:underline">← Back to my requests</a>
            <a href="{{ route('work-from-home.team') }}" class="text-sm text-indigo-600 hover:underline">View full team history →</a>
        </div>
        ...
        <div class="overflow-hidden overflow-x-auto rounded-lg bg-white shadow-sm">
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($requests as $r)
                        <tr>
                            ...
                            <td class="px-4 py-2">
                                ...
                                <form x-cloak x-show="rejecting === {{ $r->id }}" method="POST" action="{{ route('work-from-home.reject', $r) }}" class="mt-2 flex items-center gap-2">
                                    @csrf
                    ...
                    @empty
                        <tr><td colspan="6" class="px-4 py-6 text-center text-gray-400">No pending WFH requests.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
