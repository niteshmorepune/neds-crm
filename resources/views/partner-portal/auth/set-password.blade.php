<x-partner-portal-guest-layout>
    ...
    <p class="mt-1 text-sm text-gray-500">Choose a password to activate your partner portal access.</p>

    @if ($errors->any())
        <div class="mt-4 rounded-md bg-red-50 border border-red-200 px-3 py-2 text-sm text-red-700">{{ $errors->first() }}</div>
    @endif

    <form method="POST" action="{{ route('partner-portal.password.store', $token) }}" class="mt-6 space-y-4">
        @csrf
        <div>
