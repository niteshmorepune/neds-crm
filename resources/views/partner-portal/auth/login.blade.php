<x-partner-portal-guest-layout>
    ...
    <p class="mt-1 text-sm text-gray-500">Access your referrals and content submissions.</p>

    @if ($errors->any())
        <div class="mt-4 rounded-md bg-red-50 border border-red-200 px-3 py-2 text-sm text-red-700">{{ $errors->first() }}</div>
    @endif

    <form method="POST" action="{{ route('partner-portal.login') }}" class="mt-6 space-y-4">
        @csrf
        <div>
    ...
    <p class="mt-4 text-center text-sm text-gray-500">
        <a href="{{ route('partner-portal.password.forgot') }}" class="text-indigo-600 hover:text-indigo-500">Forgot your password?</a>
    </p>
</x-partner-portal-guest-layout>
