<x-app-layout>
    <x-slot name="header">Team Leave Records</x-slot>

    <div class="max-w-6xl mx-auto space-y-4">
        <div class="flex flex-wrap items-center justify-between gap-2">
            <a href="{{ route('leave-requests.approvals') }}" class="text-sm text-indigo-600 hover:underline">← Back to pending approvals</a>
        </div>
        ...
        <form method="GET" class="flex flex-wrap items-end gap-2 rounded-lg bg-white p-4 shadow-sm">
            ...
            @if (array_filter($filters))
                <a href="{{ route('leave-requests.team') }}" class="text-sm text-gray-500 hover:underline">Clear</a>
            @endif
        </form>
