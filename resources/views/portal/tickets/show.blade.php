<x-portal-app-layout :header="$ticket->subject">
    <div class="max-w-3xl space-y-6">
        ...
        <div class="rounded-lg bg-white p-6 shadow-sm">
            <p class="text-sm text-gray-500">{{ $ticket->priority->label() }} · {{ $ticket->status->label() }} · raised {{ $ticket->created_at->timezone(config('app.timezone'))->format('d M Y') }}</p>
                ...
                @forelse ($ticket->replies as $reply)
                    <li class="rounded-md border-l-2 pl-3 {{ $reply->isFromCustomer() ? 'border-blue-300 bg-blue-50' : 'border-gray-200' }}">
                        <div class="py-2">
                            ...
                            <div class="mt-0.5 text-xs text-gray-400">{{ $reply->authorName() }} · {{ $reply->created_at->timezone(config('app.timezone'))->format('d M Y, g:i A') }}</div>
                        </div>
                    </li>
                @empty
