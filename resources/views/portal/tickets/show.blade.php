<x-portal-app-layout :header="$ticket->subject">
    <div class="max-w-3xl space-y-6">
        ...
        <div class="rounded-lg bg-white p-6 shadow-sm">
            <p class="text-sm text-gray-500">{{ $ticket->priority->label() }} · {{ $ticket->status->label() }} · raised {{ $ticket->created_at->timezone(config('app.display_timezone'))->format('d M Y') }}</p>
        ...
        <div class="rounded-lg bg-white p-6 shadow-sm">
                ...
                @forelse ($ticket->replies as $reply)
                    <li class="rounded-md border-l-2 pl-3 {{ $reply->isFromCustomer() ? 'border-blue-300 bg-blue-50' : 'border-gray-200' }}">
                        <div class="py-2">
                            ...
                            <div class="mt-0.5 text-xs text-gray-400">{{ $reply->authorName() }} · {{ $reply->created_at->timezone(config('app.display_timezone'))->format('d M Y, g:i A') }}</div>
                        </div>
                    </li>
                @empty
            ...
            @if ($ticket->status->isOpen())
                <form method="POST" action="{{ route('portal.tickets.reply', $ticket->id) }}" class="mt-4 border-t border-gray-100 pt-4">
                    @csrf
                    <textarea name="body" rows="3" placeholder="Write a reply…" class="block w-full rounded-md border-gray-300 text-sm shadow-sm" required></textarea>
        ...
        <a href="{{ route('portal.tickets.index') }}" class="text-sm text-gray-500 hover:text-gray-700">← Back to tickets</a>
    </div>
</x-portal-app-layout>
