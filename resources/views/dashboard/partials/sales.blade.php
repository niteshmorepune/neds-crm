...
@if (in_array('followups_due', $visibleWidgets) || in_array('won_this_month', $visibleWidgets))
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        @endif
        @if (in_array('won_this_month', $visibleWidgets))
            <div class="rounded-lg bg-white p-5 shadow-sm">
                <div class="flex items-start justify-between gap-2">
                    <a href="{{ route('deals.index') }}" class="block hover:underline">
                        <p class="text-sm text-gray-500">Won {{ $monthLabel }}</p>
...
@if (in_array('pipeline_table', $visibleWidgets))
...
@if (in_array('call_priority_list', $visibleWidgets))
    <livewire:call-priority-list :rows="$callPriority" />
@endif

@if (in_array('overdue_follow_ups', $visibleWidgets))
